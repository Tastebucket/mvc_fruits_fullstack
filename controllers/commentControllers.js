...
router.delete('/delete/:fruitId/:commId', (req, res) => {
    //isolate ids and save to variables so we don't have to keep typing req.params
    const { fruitId, commId } = req.params
    //get fruit
    Fruit.findById(fruitId)
        .then(fruit => {
            //get comment, using built in subdoc method called .id()
            const theComment = fruit.comments.id(commId)
            console.log('this is the comment to be deleted: \n', theComment)
            //make sure user is logged in and they are author
            if (req.session.loggedIn) {
